...
export async function fetchClinics(): Promise<Clinic[]> {
  ...
  while (true) {
    ...
    if (error) {
      console.error("Error fetching clinics:", error.message, error.details, error.hint, error.code);
      break;
    }
...
export async function fetchClinicById(id: string): Promise<Clinic | null> {
  ...
  if (error) {
    console.error("Error fetching clinic:", error.message, error.details, error.hint, error.code);
    return null;
  }
...
async function geocodeLocation(location: string): Promise<{ lat: number; lng: number } | null> {
  try {
    let apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || null;
    ...
    if (!apiKey) {
      console.warn("No Google Maps API key available — geocoding skipped");
      return null;
    }
...
export async function searchClinics(query: string): Promise<{ clinics: Clinic[]; searchLocation: { lat: number; lng: number } | null }> {
  ...
  if (!cleanedQuery) return { clinics: [], searchLocation: null };
  ...
  if (!postalAnalysis.isPostalCode) {
    ...
    if (searchLocation) shouldLoadAllClinics = true;
  }
  ...
  while (true) {
    ...
    if (shouldLoadAllClinics) {
      // no filter — radius filtering done client-side
    ...
    if (error) {
      console.error("Error searching clinics:", error.message, error.details, error.hint, error.code);
      break;
    }
  ...
  if (!searchLocation && clinics.length > 0) {
    searchLocation = { lat: clinics[0].latitude, lng: clinics[0].longitude };
  }
